...
func SetInterfaceBandwidth(podName, podNamespace, iface, ingress, egress, podPriority string) error {
	...
	for _, ifName := range interfaceList {
		...
		if egressBPS > 0 {
			...
			if err = SetQosQueueBinding(podName, podNamespace, ifName, qosList, queueList); err != nil {
				return err
			}
		} else {
			for _, qos := range qosList {
...
func ClearHtbQosQueue(podName, podNamespace string) error {
	queueList, err := ovsFind("queue", "_uuid", fmt.Sprintf(`external-ids:iface-id="%s.%s"`, podName, podNamespace))
	if err != nil {
		return err
	}
...
func IsHtbQos(podName, podNamespace string) (bool, error) {
	qosList, err := ovsFind("qos", "_uuid", fmt.Sprintf(`external-ids:iface-id="%s.%s"`, podName, podNamespace))
	if err != nil {
		return false, err
	}
...
func SetQosQueueBinding(podName, podNamespace, ifName string, qosList, queueList []string) error {
	var qosCommandValues []string
	qosCommandValues = append(qosCommandValues, fmt.Sprintf("queues:0=%s", queueList[0]))
	if len(qosList) == 0 {
		qosCommandValues = append(qosCommandValues, "type=linux-htb", fmt.Sprintf(`external-ids:iface-id="%s.%s"`, podName, podNamespace))
		if podNamespace != "" && podName != "" {
...
// ClearPortQosBinding remove qos related to this port.
func ClearPortQosBinding(podName, podNamespace string) error {
...
func SetNetemQos(podName, podNamespace, iface, latency, limit, loss string) error {
	...
	for _, ifName := range interfaceList {
		...
		if latencyMs > 0 || limitPkts > 0 || lossPercent > 0 {
			if len(qosList) == 0 {
				qosCommandValues = append(qosCommandValues, "type=linux-netem", fmt.Sprintf(`external-ids:iface-id="%s.%s"`, podName, podNamespace))
				if podNamespace != "" && podName != "" {
					qosCommandValues = append(qosCommandValues, fmt.Sprintf("external-ids:pod=%s/%s", podNamespace, podName))
				}
			...
			for _, qos := range qosList {
				...
				if err = ClearPortQosBinding(podName, podNamespace); err != nil {
					klog.Errorf("failed to delete qos bingding info for interface %s/%s: %v", podNamespace, podName, err)
					return err
				}

				// reuse this function to delete qos record
				if err = ClearPodBandwidth(podName, podNamespace); err != nil {
